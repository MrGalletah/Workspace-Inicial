...
// Agregar productos al carro
  ...
  const productHTML = `
    ...
    <div class="col-1 col-sm-1 p-0 text-center p-0 removeItem" title="Eliminar del carrito" data-productID="${productID}"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="red" class="bi bi-trash3" viewBox="0 0 16 16">
...
function checkProductQuantity(){
  ...
  let inputsChecked = [];
  for(const productQuantityInput of productQuantityInputs) { 
 if(ProductQuantityValidation(productQuantityInput.value)) {
 const inputNotValidated = {
  check: true
 };
inputsChecked.push(inputNotValidated);
 } else {
  const inputValidated = {
    check: false
  };
  inputsChecked.push(inputValidated);
 }
  }
  return inputsChecked;
};

function validateInputsChecked() {
 const inputsChecked = checkProductQuantity()
 if(inputsChecked.find(input => input.check === false)){
  return false;
 } else {
  return true;
 }
...
function ProductQuantityValidation(value){
  if(value !== '' && value >= 1){
    return true;
  } else {
    return false;
...
if(streetAddress.value !== '' && numberAddress.value !== '' && cornerAddress.value !== '' && selectedOption !== "disabled" && validateMethodPayment(methodPaymentSelected) && validateInputsChecked()) {
